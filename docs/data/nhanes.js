...
const NHANES = (() => {
  ...
  const AGE_GROUPS = ["18-29","30-39","40-49","50-59","60-69","70-79","80+"];
  const SEX = ["Male","Female"];
  const ETHNICITY = ["Non-Hispanic White","Non-Hispanic Black","Mexican-American","Other Hispanic","Asian","Other/Multi"];

  // ── Blood Pressure (BPX/BPXO datasets, 1999-2023) ──
  // Systolic BP distributions by age group and sex (mmHg)
  // Format: { mean, sd, p5, p25, p50, p75, p95 }
  const BP_SYSTOLIC = {
    "Male": {
  ...
  // BP by ethnicity (all ages combined, both sexes)
  const BP_BY_ETHNICITY = {
  ...
  // Hypertension prevalence (SBP>=140 or DBP>=90) by age group
  const HYPERTENSION_PREVALENCE = {
    "18-29": 0.078, "30-39": 0.123, "40-49": 0.218,
    "50-59": 0.358, "60-69": 0.478, "70-79": 0.592, "80+": 0.641
  };

  // BP trends across survey cycles (population mean SBP)
  const BP_TRENDS = {
  ...
  const SCORING_THRESHOLDS = {
    qSOFA: {
      ...
      sensitivity: 0.70, specificity: 0.79, auc: 0.81
    },
    sirs: {
      name: "SIRS Criteria",
  ...
  function getPercentile(value, distribution) {
    if (value <= distribution.p5) return Math.round((value - distribution.mean + 2 * distribution.sd) / (4 * distribution.sd) * 5);
    if (value <= distribution.p25) return 5 + Math.round((value - distribution.p5) / (distribution.p25 - distribution.p5) * 20);
    if (value <= distribution.p50) return 25 + Math.round((value - distribution.p25) / (distribution.p50 - distribution.p25) * 25);
    if (value <= distribution.p75) return 50 + Math.round((value - distribution.p50) / (distribution.p75 - distribution.p50) * 25);
    if (value <= distribution.p95) return 75 + Math.round((value - distribution.p75) / (distribution.p95 - distribution.p75) * 20);
    return Math.min(99, 95 + Math.round((value - distribution.p95) / distribution.sd * 2));
  }

  function getBPBenchmark(sbp, age, sex) {
  ...
  function getHRBenchmark(hr, age, sex) {
    const ageGroup = getAgeGroup(age);
  ...
  function getCompleteBenchmark(vitals, age, sex) {
    const results = {};
    if (vitals.sbp != null) results.sbp = getBPBenchmark(vitals.sbp, age, sex);
    if (vitals.heart_rate != null) results.heart_rate = getHRBenchmark(vitals.heart_rate, age, sex);
  ...
  function getRiskProfile(age, sex, ethnicity, comorbidities) {
    const ageGroup = getAgeGroup(age);
    let baseRisk = SEPSIS_EPIDEMIOLOGY.incidence_per_100k[ageGroup] / 100000;
    let multiplier = 1.0;
    const factors = [];
    ...
    return {
      baseRisk, multiplier, adjustedRisk, factors, ageGroup,
  ...
  function generateDistributionCurve(dist, numPoints) {
  ...
  // ── Public API ──
  return {
    CYCLES, AGE_GROUPS, SEX, ETHNICITY,
    BP_SYSTOLIC, BP_DIASTOLIC, BP_BY_ETHNICITY,
    HYPERTENSION_PREVALENCE, BP_TRENDS,
    HEART_RATE, RESPIRATORY_RATE, TEMPERATURE, SPO2,
    WBC, HEMOGLOBIN, PLATELETS,
    CRP, CRP_BY_BMI,
    BMI, OBESITY_TRENDS,
    SEPSIS_RISK_FACTORS, SEPSIS_EPIDEMIOLOGY,
    SCORING_THRESHOLDS, POPULATION_SCORE_DISTRIBUTION,
    getPercentile, getBPBenchmark, getHRBenchmark,
    getCompleteBenchmark, getRiskProfile,
    generateDistributionCurve, getAgeGroup
  };
})();

if (typeof module !== "undefined" && module.exports) module.exports = NHANES;
